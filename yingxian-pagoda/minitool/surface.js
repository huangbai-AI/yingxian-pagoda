...
const texture=(name,color=false)=>{const t=loader.load('./textures/'+name);t.wrapS=t.wrapT=THREE.RepeatWrapping;t.repeat.set(.82,.82);t.flipY=false;t.colorSpace=color?THREE.SRGBColorSpace:THREE.NoColorSpace;return t;};
const timberColor=texture('timber-color.webp',true),timberNormal=texture('timber-normal.webp'),timberRough=texture('timber-roughness.webp');
const wallColor=texture('wall-color.webp',true),wallNormal=texture('wall-normal.webp');
// Relief follows the eight original roof slopes; the curved eaves stay geometry.
const tileCanvas=document.createElement('canvas');tileCanvas.width=128;tileCanvas.height=256;
const tileCtx=tileCanvas.getContext('2d'),tilePixels=tileCtx.createImageData(128,256);
for(let y=0;y<256;y++)for(let x=0;x<128;x++){const wave=Math.pow(Math.max(0,Math.cos((x/128-.5)*Math.PI)),3);const seam=y<8?.62:1;const h=Math.round((50+wave*175)*seam);const k=(y*128+x)*4;tilePixels.data.set([h,h,h,255],k);}
tileCtx.putImageData(tilePixels,0,0);const tileHeight=new THREE.CanvasTexture(tileCanvas);tileHeight.wrapS=tileHeight.wrapT=THREE.RepeatWrapping;
...
const stoneTexture=new THREE.CanvasTexture(stoneCanvas);stoneTexture.wrapS=stoneTexture.wrapT=THREE.RepeatWrapping;
stoneTexture.repeat.set(5,5);stoneTexture.colorSpace=THREE.SRGBColorSpace;
export function refineSurface(mesh,renderer){
 const m=mesh.material;
 m.alphaHash=false;m.alphaToCoverage=true;m.transparent=false;m.opacity=1;
 // Adjust colors for timber materials; keep GLB-embedded textures
 if(/古木|浅木|暗木|朱漆/.test(m.name)){
  for(const t of [timberColor,timberNormal,timberRough])t.anisotropy=Math.min(4,renderer.capabilities.getMaxAnisotropy());
  m.map=timberColor;m.normalMap=timberNormal;m.normalScale.set(.36,.36);m.roughnessMap=timberRough;m.bumpMap=null;
  m.roughness=.87;m.metalness=0;
 ...
 if(/风化土朱墙/.test(m.name)){m.color.setRGB(.65,.65,.65);m.map=wallColor;m.normalMap=wallNormal;m.normalScale.set(.45,.45);m.roughness=.98;m.metalness=0;}
 if(/青灰瓦/.test(m.name)){m.color.setRGB(.075,.09,.095);m.bumpMap=tileHeight;m.bumpScale=.032;m.roughness=.94;}
 ...
 // Reveal shader for isolation/explode animation
