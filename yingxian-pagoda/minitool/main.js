...
function supportsFlexGap(){const test=document.createElement('div');test.style.position='absolute';test.style.visibility='hidden';test.style.display='flex';test.style.flexDirection='column';test.style.rowGap='1px';test.appendChild(document.createElement('div'));test.appendChild(document.createElement('div'));document.body.appendChild(test);const supported=test.scrollHeight===1;test.parentNode.removeChild(test);return supported;}
...
let lenis,renderer,cinema,controls,model,loaded=false,exploring=false,lastFocus,cutaway=false,hiddenRoof=false,autorotate=false,manualExplode=0,selectedFloor='all',planTop=false,fallbackActive=false,qualityReduced=false;
let savedScroll=0;
let rafId=null,rafRunning=false;
function resizeStage(){if(!renderer)return;viewportW=Math.max(1,stage.clientWidth);viewportH=Math.max(1,stage.clientHeight);document.documentElement.style.setProperty('--app-height',innerHeight+'px');const pixelBudget=qualityReduced?1000000:2000000;const budgetRatio=Math.sqrt(pixelBudget/(viewportW*viewportH));const qualityCap=qualityReduced?1:1.5;renderer.setPixelRatio(Math.min(devicePixelRatio||1,qualityCap,budgetRatio));renderer.setSize(viewportW,viewportH,false);cinema.resize(viewportW,viewportH);camera.aspect=viewportW/viewportH;camera.updateProjectionMatrix();if(lenis&&!lenis.options.autoResize)lenis.resize();}
...
const key=new THREE.DirectionalLight(0xffd39a,5.8);key.position.set(55,80,50);key.castShadow=false;key.shadow.mapSize.set(1024,1024);key.shadow.normalBias=.045;key.shadow.bias=-.00035;key.shadow.radius=4;key.shadow.blurSamples=4;key.shadow.camera.near=1;key.shadow.camera.far=220;scene.add(key,key.target);
...
function setupMotion(){lenis?.destroy();lenis=null;document.body.classList.toggle('reduced-motion',reduced);$('#motion-toggle').setAttribute('aria-pressed',String(reduced));$('#motion-toggle').textContent=reduced?'恢复动态效果':'减少动态效果';if(!reduced){lenis=new Lenis({duration:1.05,smoothWheel:true,anchors:true,autoResize:typeof ResizeObserver!=='undefined'});lenis.on('scroll',ScrollTrigger.update);}}
...
function failure(error){
 if(fallbackActive)return;if(exploring)closeExplorer();
 console.warn('三维场景已切换到静态导览',error);fallbackActive=true;loaded=false;stopRAF();document.body.classList.remove('loading');lenis?.start();$('#model-loading').hidden=true;$('#model-error').hidden=false;$('#fallback-poster').hidden=false;canvas.hidden=true;
 arrivalTimeline?.kill();$('#arrival-shade').style.opacity=0;gsap.set($$('.hero h1 span,.hero-meta,.hero-note,.scroll-cue'),{clearProps:'opacity,visibility,transform,filter'});$('#column-hotspot').hidden=true;$('#label-lines').style.display='none';$('.object-labels').hidden=true;
 ['#open-explorer','#explore-end','#explore-footer','#inspect-structure','#plan-close'].forEach(id=>{const el=$(id);if(el)el.disabled=true;});
 let lastPoster=-1;
 gsap.ticker.add(()=>{if(!fallbackActive||document.hidden)return;updateText();updateAtmosphere(state.phase);const poster=$('#fallback-poster'),index=clamp(Math.round(state.phase),0,5);if(index!==lastPoster){poster.src='./posters/chapter-'+index+'.webp';lastPoster=index;}const env=index===0||index===5?1:0;document.documentElement.style.setProperty('--intro-env',index===0?1:0);document.documentElement.style.setProperty('--guard-env',index===5?1:0);document.documentElement.style.setProperty('--environment',env);poster.style.transform='translateY('+((state.phase-index)*-18)+'px)';});
}
$('#retry').onclick=()=>location.reload();document.body.classList.add('loading');lenis?.stop();
try{
 renderer=new THREE.WebGLRenderer({canvas,alpha:true,antialias:true,powerPreference:'high-performance'});renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.12;renderer.setClearColor(0x10201e,0);renderer.shadowMap.enabled=false;renderer.localClippingEnabled=true;renderer.info.autoReset=false;
 ...
 canvas.addEventListener('webglcontextlost',e=>{e.preventDefault();failure(new Error('图形环境已暂停'));},false);
 ...
 resizeStage();
 if(typeof ResizeObserver!=='undefined')new ResizeObserver(resizeStage).observe(stage);else window.addEventListener('resize',resizeStage);
 requestAnimationFrame(()=>requestAnimationFrame(async()=>{try{
  $('#load-percent').textContent='68%';$('#load-bar').style.width='68%';
  model=await buildPagoda(p=>{$('#load-percent').textContent=Math.round(68+p*30)+'%';$('#load-bar').style.width=Math.round(68+p*30)+'%';});model.name='应县木塔';scene.add(model);model.updateMatrixWorld(true);
  const isMob=mobile();
  model.traverse(o=>{
   if(/^Level_\d+$/.test(o.name)){o.userData.index=Number(o.name.split('_')[1]);o.userData.baseY=o.position.y;levels.push(o);}
   if(!o.isMesh)return;meshes.push(o);o.castShadow=false;o.receiveShadow=false;o.material=o.material.clone();const m=o.material;m.alphaHash=true;m.roughness=.81;m.clippingPlanes=[];
   ...
   if(/青灰瓦|瓦脊/.test(m.name))m.color.setRGB(.11,.13,.14);if(/石台/.test(m.name))m.color.setRGB(.12,.14,.13);
   refineSurface(o,renderer);
   if(/牌匾墨书/.test(m.name)){m.color.setHex(0xffffff);m.polygonOffset=true;m.polygonOffsetFactor=-1;m.polygonOffsetUnits=-2;m.roughness=.94;m.metalness=0;m.bumpMap=null;m.normalMap=null;m.map.anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy());o.castShadow=false;}
  ...
  window.__pagoda={loaded:true,lightweight:true,levels:levels.length,meshes:meshes.length,stats:()=>({lighting:{key:key.intensity,fill:fill.intensity,ambient:ambient.intensity},triangles:renderer.info.render.triangles,calls:renderer.info.render.calls,frameMs:frames.reduce((a,b)=>a+b,0)/(frames.length||1),qualityReduced,phase:state.phase,exploring,explode:state.explode,hiddenRoof,cutaway,floor:selectedFloor,fov:camera.fov,camera:camera.position.toArray(),target:controls.target.toArray(),levelOffsets:levels.map(l=>({level:l.userData.index,y:l.position.y})),connected:state.explode<.001&&roofMeshes.every(m=>Math.abs(m.position.y)<.001)})};
 }catch(error){failure(error);}}));
	}catch(e){failure(e);}
async function startArrival(){
 // 等字体和第一帧就绪，避免退场后突然出现空塔或换字。
 const fontsReady=document.fonts&&document.fonts.ready?document.fonts.ready:Promise.resolve();
 await Promise.race([fontsReady,new Promise(r=>setTimeout(r,1800))]);
 ...
 tiers.forEach(el=>{if(el.getAnimations)el.getAnimations().forEach(animation=>animation.pause());});
 const tierFrames=tiers.map(el=>{const style=getComputedStyle(el);return {transform:style.transform,opacity:style.opacity};});
 tiers.forEach((el,i)=>{if(el.getAnimations)el.getAnimations().forEach(animation=>animation.cancel());el.style.animation='none';if(el.animate)el.animate([tierFrames[i],{transform:'translateY(0px)',opacity:1}],{duration:280,easing:'ease-out',fill:'forwards'});else{el.style.transform='translateY(0px)';el.style.opacity=1;}});
...
buildStory();window.addEventListener('resize',()=>{buildStory();ScrollTrigger.refresh();});if(document.fonts&&document.fonts.ready)document.fonts.ready.then(()=>ScrollTrigger.refresh());
...
const poses=[
 {pos:[18,18,60],aim:[0,30,0],fov:64,shift:0,roll:0,explode:0,isolate:0,roof:0,env:1},
...
for(const [i,pose] of poses.entries()){
 pose.fov??=34;const offset=i>=1&&i<=3?8:0,delta=modelHeight(pose.aim[1]-offset)+offset-pose.aim[1];
...
function updateAssembly(s,phase){
 ...
 // Use the camera's story range, not the opacity of the other floors. The
 // return shot keeps closed bracket solids before switching to distant detail.
 const detail=qualityReduced?0:exploring?(selectedFloor==='3'?1:0):phase>1.65&&phase<3.5?1:phase>=3.5&&phase<4.08?2:0;
 ...
 meshes.forEach(m=>{
  ...
  if(m.userData.stair)opacity*=1-Math.max(state.explode,exploring?0:s.isolate);if(exploring&&hiddenRoof&&m.userData.roof)opacity=0;
  if(i===3&&m.userData.detail!==detail)opacity=0;
  m.position.y=lift;m.material.opacity=1;m.material.userData.reveal.value=opacity;m.visible=opacity>.015;
 });
...
function setCamera(s){
 ...
 if(mobile()){
  const factors=[1.25,1.55,2.55,1.8,1.65,1.65],xs=[0,-.12,.02,.11,.16,-.25],ys=[.07,-.11,-.10,.05,-.07,-.04];const a=Math.min(4,Math.floor(state.phase)),t=ease(clamp((state.phase-a-.08)/.84,0,1)),b=a+1;cameraPosition.sub(cameraAim).multiplyScalar(lerp(factors[a],factors[b],t)).add(cameraAim);shift=lerp(xs[a],xs[b],t);shiftY=lerp(ys[a],ys[b],t);
 }
...
function setLabels(phase){
 ...
 const paths=[],angPoint=project(0,35.80,12.18);
 [['#label-dou',0,36.47,11.40],['#label-gong',.55,35.94,11.095],['#label-ang',0,35.80,12.18]].forEach(([id,x,y,z],i)=>{
  const el=$(id),p=project(x,y,z),lx=viewportW*(mobile()?.08:.12),baseY=viewportH*(mobile()?[.56,.68,.39][i]:[.59,.72,.46][i]),ly=i===2?Math.min(baseY,angPoint.y-40):Math.max(baseY,angPoint.y+(i===0?34:110));el.style.opacity=amount;el.style.left=lx+'px';el.style.top=ly+'px';
  const elbow=lx+(mobile()?48:80);
  paths.push(`<path d="M${lx+28} ${ly} H${elbow} L${p.x} ${p.y}"/><circle cx="${p.x}" cy="${p.y}" r="2"/>`);
...
function updateAtmosphere(phase){
 ...
 const backgroundLight=[1,.75,.90,.88,1.08,1],interiorTexture=range(.15,.85,phase)*(1-range(4.15,4.85,phase));// 淡纹理只出现在中间四幕，首尾自然退净。
 style.setProperty('--interior-texture',interiorTexture);style.setProperty('--texture-haze-opacity',interiorTexture*.28);style.setProperty('--texture-grain-opacity',interiorTexture*.055);
 style.setProperty('--backdrop-light',lerp(backgroundLight[a],backgroundLight[a+1],t));style.setProperty('--haze-x',values[0]+'%');style.setProperty('--haze-y',values[1]+'%');style.setProperty('--chamber',values[2]);style.setProperty('--chamber-opacity',values[2]*.42);style.setProperty('--haze-angle',values[3]+'deg');style.setProperty('--haze-pan',((phase-2.5)*-2.8)+'%');
}
function updateLight(phase){
 ...
 key.intensity=lerp(lerp(3.45,2.75,interior),4.25,intro);key.color.setHex(0xffc78f).lerp(new THREE.Color(0xffd9ae),interior*.7);
 scene.environmentIntensity=lerp(.10,.14,interior);
 ...
 ambient.intensity=lerp(.16,.23,interior);fill.intensity=lerp(.18,.31,interior)+close*.08;
 fill.position.set(lerp(-30,-7,close),lerp(38,y+1,close),lerp(60,z+8,close));fill.target.position.set(0,y,z);
 rim.intensity=lerp(.30,.22,interior);rim.target.position.set(0,y,z);
...
function render(now){
 ...
 const rawFrameMs=now-lastTime,dt=Math.min(rawFrameMs/1000,.1);lastTime=now;
 ...
 renderer.info.reset();cinema.render(state.phase,exploring);if(frameCount++%8===0&&rawFrameMs>0){frames.push(rawFrameMs);if(frames.length>60)frames.shift();if(!qualityReduced&&frames.length>=45&&frames.reduce((a,b)=>a+b,0)/frames.length>42){qualityReduced=true;failure(new Error('设备持续掉帧，保留画面细节并切换为图文导览'));}}
}
function startRAF(){if(rafRunning||fallbackActive||document.hidden)return;rafRunning=true;lastTime=performance.now();rafId=requestAnimationFrame(render);}
function stopRAF(){rafRunning=false;if(rafId)cancelAnimationFrame(rafId);}
startRAF();
document.addEventListener('visibilitychange',()=>{if(document.hidden){stopRAF();gsap.ticker.sleep();}else{gsap.ticker.wake();startRAF();}});
...
function poseView(view='overall',animate=true){
 if(!loaded)return;$$('[data-view]').forEach(b=>b.classList.toggle('active',b.dataset.view===view));const floor=selectedFloor==='all'?null:Number(selectedFloor),fy=modelHeight([2,12,25,34,43,52,62][floor??0])+offsetFor(floor??2,manualExplode);let aim=new THREE.Vector3(0,floor===null?modelHeight(32):fy,0),pos=new THREE.Vector3(65,60,145);
